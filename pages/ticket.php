...
//		$date = date('M m Y', time());
		...
		$SQL->query("INSERT INTO `tickets`(`ticket_id`, `ticket_subject`, `ticket_author`, `ticket_author_acc_id`,`ticket_last_reply`, `ticket_admin_reply`,`ticket_date`, `ticket_ended`, `ticket_status`, `ticket_category`, `ticket_description`)	VALUES ($generateId,'$subject','$playerName',$accid,'You',0,'$date','Not closed','Aguardando','$category','$description')");
	...
	if ($action == "showticket") {
        ...
        if ($metodo == 'closeticket') {
            $date = date('M m Y', time());
            $SQL->query("UPDATE tickets SET ticket_status = 'Fechado', ticket_ended = '$date' WHERE ticket_id = $idTicket");
        }
        ...
        if ($metodo == 'reply') {
            ...
            if (strlen($mensagem) < 10 || strlen($mensagem) > 1000) {
                ...
                if ($group_id_of_acc_logged >= $config['site']['access_admin_panel']) {
                    $SQL->query("UPDATE `tickets` SET `ticket_last_reply` = 'Staff', `ticket_admin_reply` = 1, `ticket_status` = 'Respondido' WHERE ticket_id = $idTicket");
                } else {
                    $SQL->query("UPDATE `tickets` SET `ticket_admin_reply` = 0, `ticket_last_reply`= 'You', `ticket_status` = 'Aguardando' WHERE ticket_id = $idTicket");
                }
            }
        }
		...
		if ($status == 'Respondido') {
			$main_content .= '<td><font color="#32cd32"><b>' . $status . '</b></font></td>';
		}
        if ($status == 'Fechado') {
